...
class AnsiKit {
	 ...
	 * @param   {number} x=1 How much lines to move down
	 * @return  {AnsiKit}
	 */
	static cursorDown(x) {
	 ...
	 * @param   {number} x=1 How much lines to move up
	 * @return  {AnsiKit}
	 */
	static cursorUp(x) {
		if (x === -1) x = process.stdout.rows;
	...
	static printCode(code, terminate) {
		process.stdout.write(`\x1b[${code}${terminate ? '\x1b\\' : ''}`);
		return AnsiKit;
	}

	static printCSI(code, end) {
		process.stdout.write(`\x1b[${code}${end === undefined ? 'm' : end}`);
		return AnsiKit;
	}
